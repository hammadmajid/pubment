...
const commentController = {
    create: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            if (!validationResult.success) {
                res.status(400).json(commentErrorResponse.parse({
                    success: false,
            ...
            if (!post) {
                res.status(404).json(commentErrorResponse.parse({
                    success: false,
    ...
    getById: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            const comment = await Comment.findById(id).populate('author', 'username name profilePicture');
            if (!comment) {
                res.status(404).json(commentErrorResponse.parse({
                    success: false,
